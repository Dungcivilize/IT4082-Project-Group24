...
const ManageHousehold = () => {
  ...
  const fetchHouseholds = async () => {
    try {
      ...
      const response = await fetch(
        'http://localhost:8080/api/ownerships/active'
      );
  ...
  const fetchResidents = async (ownershipId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/ownerships/${ownershipId}/residents`
      );
  ...
  const fetchVehicles = async (ownershipId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/ownerships/${ownershipId}/vehicles`
      );
  ...
  const fetchTemporaryResidents = async (ownershipId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/ownerships/${ownershipId}/temporary-residents`
      );
  ...
  const fetchTemporaryAbsents = async (ownershipId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/ownerships/${ownershipId}/temporary-absents`
      );
  ...
  const handleConfirmMoveOut = async () => {
    ...
    try {
      ...
      const ownershipRes = await fetch(
        `http://localhost:8080/api/ownerships/${confirmMoveOut.ownershipId}`,
        {
          method: 'PUT',
      ...
      const responseData = await ownershipRes.json();

      if (!ownershipRes.ok) {
        // Nếu server trả về lỗi, hiển thị message từ server
        setError(
          responseData.message || 'Có lỗi xảy ra khi cập nhật trạng thái'
        );
        return;
      }
        ...
        {!selectedHousehold ? (
              ...
              >
                <div className="household-header">
                  ...
                  <div className="dropdown-container" ref={dropdownRef}>
                    <button
                      ...
                      onClick={(e) =>
                        handleDropdownToggle(e, household.ownershipId)
                      }
                    >
                ...
                <div className="household-info">
                  ...
                  <div className="info-item">
                    <span>
                      Tầng:{' '}
                      <span className="info-value">{household.floor}</span>
                    </span>
                  </div>
                  <div className="info-item">
                    <span
          ...
          <div className="detail-view">
            ...
            <div className="tabs">
              ...
              <div
                className={`tab ${
                  activeTab === 'temporaryResidence' ? 'active' : ''
                }`}
              ...
              <div
                className={`tab ${
                  activeTab === 'temporaryAbsence' ? 'active' : ''
                }`}
        ...
        <div className="modal-overlay">
          <div className="confirmation-modal">
            ...
            <p className="warning-text">
              ...
              <br />• Đặt trạng thái căn hộ thành "Trống"
            </p>
